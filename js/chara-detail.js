    ...
    const IMAGE_REPO = "https://puppiizsunniiz.github.io/Arknight-Images"
    const SPINE_REPO = "https://puppiizsunniiz.github.io/Arknights-Spines"
    ...
    var folder = `${SPINE_REPO}/spineassets/${chibitype}/${charName}/${chibipers}/`
    var spinewidget
    var spinewidgetcg
    var opid;
    var curropid
    ...
    $(document).ready(function(){
        ...
        db.all_chars = {...db.chars, ...db.charpatch.patchChars}
        db.all_charsEN = {...db.charsEN, ...db.charpatchEN.patchChars}

        Object.keys(db.all_chars).forEach(id =>{
            var currentop = db.all_chars[id]
            ...
            if(id != "none"){
                teamHTML[db.handbookTeam[id].powerLevel].push(
                    `<div class="op-faction btn-secondary tooltip2 adv-filter" data-id="${id}" onclick="toggleBtn(this)" section="faction">
                        <img loading='lazy' class='filter-img faction-img' src='${IMAGE_REPO}/factions/logo_${id}.png'>
        ...
        $("#Chibi-Show").click(function(){
            ...
            if(!loadchibi){
                ...
                if(bgnum == 0 && $("#spine-bg").is(":hidden")){
                    bgnum = 1
                    $('#spine-bg').attr("src", `${IMAGE_REPO}/ui/spine/bg${bgnum}.png`);
                    ...
                    case "List":
                                ...
                                `
                                    <li class='selectop-list ak-shadow' onclick='selectOperator("${key}")'>
                                        <img loading='lazy' src='${IMAGE_REPO}/avatars/${key}.png'>
                    ...
                    case "Grid":
                                ...
                                `
                                    <li class='selectop-grid ak-shadow' onclick='selectOperator("${key}")'>
                                        ...
                                        <div class='ak-showsubclass'><img loading='lazy' src='${IMAGE_REPO}/ui/subclass/sub_${val.subProfessionId}_icon.png'></div>
                                        ${cname == "" && classlogo?`<div class='ak-showclass'><img loading='lazy' src='${IMAGE_REPO}/classes/class_${classlogo}.png'></div>`:""}
                                        ${showtype && camplogo?`<div class='ak-showfaction'><img loading='lazy' src='${IMAGE_REPO}/factions/${camplogo.toLowerCase()}.png' title='${db.campdata[camplogo]}' ></div>`:""}
    ...
    function populateOperators(el, isenter = false){
        ...
        if(el.value != "" || el == "Browse" || el == "Browse2" || isenter && el){
            ...
            if(result.length > 0){
                ...
                var rarity = -1
                for (var i = 0; i < result.length; i++) {
                    let image = `<img loading='lazy' style="height:40px;padding:2px" src="${IMAGE_REPO}/avatars/${result[i].id+(sus && RIEPIC.includes(result[i].id)?"_sus":"")}.png">  `
                    // console.log(image)
                    $("#operatorsResult").removeClass("opbrowse1");
                    $("#operatorsResult").removeClass("opbrowse2");
                    if(el == "Browse3"){
                        image = `<img loading='lazy' class='opres-img' src="${IMAGE_REPO}/avatars/${result[i].id+(sus && RIEPIC.includes(result[i].id)?"_sus":"")}.png">  `
                        ...
                        image = `<img loading='lazy' src="${IMAGE_REPO}/avatars/${result[i].id+(sus && RIEPIC.includes(result[i].id)?"_sus":"")}.png">  `
                        $("#operatorsResult").css("text-align", "center");
                        $("#operatorsResult").removeClass("opresult-list");
                        $("#operatorsResult").addClass("opresult-grid");
    ...
    function getBranchclassHtml(btn) {
        function branchclassHtml(data_id, data_name) {
            ...
            <img loading='lazy' src="${`${IMAGE_REPO}/ui/subclass/sub_${data_id}_icon.png`}" style="width:33px;height:33px;object-fit: contain;display: inline-block;" alt="${data_name}">
    ...
    function actualizeFilter() {
        ...
        $("#selectedopclass").html(ops.map(char =>{
            ...
            <div class="op-image-grid">
                ${GetLogo(char)?`<div class="op-grid-faction"><img loading='lazy' src="${IMAGE_REPO}/factions/${GetLogo(char)?GetLogo(char).toLowerCase():"none"}.png" title="${GetLogo(char)?GetLogoInfo(char):"None"}"></div>`:""}
                ${gamemode=="BASE"?"":'<div class="op-grid-gamemode ' + gamemode + '" title="' + mode_title[gamemode] + '">' + gamemode + '</div>'}
                <img loading='lazy' src="${IMAGE_REPO}/avatars/${getId(char)+(sus && RIEPIC.includes(getId(char))?"_sus":"")}.png">
            </div>
            ...
            ${showfaction?`<div class='ak-showclass'><img loading='lazy' src='${IMAGE_REPO}/classes/class_${db.classes.find(search=>search.type_data==char.profession).type_en.toLowerCase()}.png'></div>`:""}
            ${op_branch.length!=1?`<div class='ak-showsubclass'><img loading='lazy' src='${IMAGE_REPO}/ui/subclass/sub_${char.subProfessionId}_icon.png'></div>`:""}
    ...
    function ChangeZoomChara(skinName, src=''){
        ...
        if(skinName != ''){
            $("#charazoom").attr("src",`${IMAGE_REPO}/characters/${skinName}${(sus && opdataFull.id == "char_298_susuro"?"sus":"")}.png`);
        }else{
    ...
    function selectOperator(opid, from = 'Selecting Operator From Browse'){
        ...
        if(opid){
            ...
            if(logo){
                        ...
                        if(!faction_all.includes(faction)){
                            faction_sub_text += `<div>
                                                    <div id="op-faction-image" style="display: inline;">
                                                        <img loading='lazy' id="op-faction-image3" src="${IMAGE_REPO}/factions/logo_${faction}.png">
                                                    </div>
                ...
                $("#op-faction").attr("src",`${IMAGE_REPO}/factions/${logo.toLowerCase()}.png`);

                $("#op-faction-btn").html(`
                    <div id="op-faction-image" style="display: inline;">
                        <img loading='lazy' id='op-faction-image2' src='${IMAGE_REPO}/factions/${logo.toLowerCase()}.png'>
                    </div>
                ...
                $("#op-faction").attr("src",`${IMAGE_REPO}/factions/none.png`)
            }
            ...
            folder = `${SPINE_REPO}/spineassets/${chibitype}/${charName}/${chibipers}/`
            // if(spinewidget)
            ...
            for (var i = 0; i < opdataFull.phases.length; i++) {
                ...
                ${dynextra?`<button class='btn tabbing-btns tabbing-btns-middle' data-toggle='pill' style='width:24px;height:25px;display:inline-block' title="Show Animated CG" onClick='ShowDynamic("${opcode}")'>
                    ...
                    <button class='btn tabbing-btns tabbing-btns-middle ${l==0?"active":""}' data-toggle='pill' style='${dynextra?"width:62px;":""}height:30px' href='#opCG_${i}_tab' onClick='ChangeSkin("${i}")'>
                        <img loading='lazy' style='max-height:30px' src='${IMAGE_REPO}/ui/elite/${i}-s.png'>
                    </button>
                </li>`);
                ...
                <li class='nav-item'>
                    <a class='btn tabbing-btns horiz-small nav-link ${i==0?"active":""} tablink' data-toggle='pill' onclick='UpdateElite(${i})'href='#elite_${i}_tab'>
                        <img loading='lazy' src="${IMAGE_REPO}/ui/elite/${i}.png" style="width:20px;margin:-12px 0px -6px 0px" title="Elite${i}">E${i}
                    </a>
                </li>`);
                ...
                //             + "<img style='max-height:30px' src='${IMAGE_REPO}/ui/elite/0-s.png' data-toggle='pill' href='#opCG_0_tab'></button></li>");
                ...
                //                             + "<img style='max-height:30px' src='${IMAGE_REPO}/ui/elite/0-s.png'></button></li>");
                //         tabbtn2[i] = $(`<li class='nav-item'><a class='btn tabbing-btns horiz-small nav-link active tablink' data-toggle='pill' onclick='UpdateElite(${i})'href='#elite_${i}_tab'><img src="${IMAGE_REPO}/ui/elite/0.png" style="width:20px;margin:-12px 0px -6px 0px" title="Elite0">E0</a></li>`);
                //     }
                // } else if( i == l-1 ){
                ...
                //                             + "<img style='max-height:30px' src='${IMAGE_REPO}/ui/elite/"+i+"-s.png'></button></li>");
                //     tabbtn2[i] = $(`<li class='nav-item'><a class='btn tabbing-btns horiz-small nav-link tablink' data-toggle='pill' style='height:30px' onclick='UpdateElite(${i})' href='#elite_${i}_tab'><img src="${IMAGE_REPO}/ui/elite/${i}.png" style="width:20px;margin:-12px 0px -6px 0px" title="Elite${i}">E${i}</a></li>`);
                // }else{
                ...
                if(skindata){
                    ...
                    zoombtn.push($(`<button class="btn ak-c-black btn-dark" style="margin:2px;padding:2px; height: 50px; width: 50px;" onclick="ChangeZoomChara('${skindata.portraitId}')"><img loading='lazy' src='${IMAGE_REPO}/ui/elite/${i}-s.png'></button>`))
                    if(i == 0){
                ...
                if(AmiyaPatchID.includes(opKey)){
                    zoombtn.push($(`<button class="btn ak-c-black btn-dark" style="margin:2px;padding:2px; height: 50px; width: 50px;" onclick="ChangeZoomChara('${opKey}')"><img loading='lazy' src='${IMAGE_REPO}/ui/elite/${i}-s.png'></button>`))
                    if(i == 0){
            ...
            if(extraSkin.length>0){
                ...
                for(var i=0;i<extraSkin.length;i++){
                    ...
                    <img loading='lazy' style="max-width:40px;max-height:40px;" src='${IMAGE_REPO}/avatars/${encodeURIComponent(extraSkin[i].avatarId)}${sus && opdataFull.id == "char_298_susuro"?"sus":""}.png'>
                    </button>`))

                    tabcontent.push($(`
                    <div class='tab-pane container' id='opCG_S${i}_tab'>
                    <img loading='lazy' class='chara-image' src='${IMAGE_REPO}/characters/${encodeURIComponent(extraSkin[i].portraitId)}${sus && opdataFull.id == "char_298_susuro"?"sus":""}.png'>
                    </div>
                    `))
                    ...
                    dropdowntab.push(`
                        ...
                        <a class="btn tabbing-btns tabbing-btns-middle" style="${extraSkin[i].dynIllustId?"width:62px":""}" data-toggle='pill' href='#opCG_S${i}_tab' onClick='ChangeSkin("${extraSkin[i].portraitId}","","${extraSkin[i].skinId}"${extraSkin[i].spIllustId?`,"#opCG_S${i}_tab"`:""})'>
                            ...
                            <img loading='lazy' class='skinimage' style="max-width: 48px;max-height: 48px;margin-left:-5px;margin-top:1px" src='${IMAGE_REPO}/avatars/${encodeURIComponent(extraSkin[i].avatarId)}${sus && opdataFull.id == "char_298_susuro"?"sus":""}.png'>
                        </a></li>
                        `)
                }
            ...
            <div>
                <img loading='lazy' class='audioprofilebutton' src="${IMAGE_REPO}/ui/story/profile.png" style="max-width:40px;max-height:40px">
            ...
            <div>
                <img loading='lazy' class='audioprofilebutton' src="${IMAGE_REPO}/ui/story/audio.png" style="max-width:40px;max-height:40px">
            ...
            $("#op-classImage").attr("src",`${IMAGE_REPO}/classes/black/icon_profession_${type['type_'+lang].toLowerCase()}_large.png`)
            $("#op-className").html(type['type_'+lang])
            $("#op-subclassImage").attr("src",`${IMAGE_REPO}/ui/subclass/sub_${opdataFull.subProfessionId}_icon.png`)
            ...
            for(i=0;i<potentials.length;i++){
                potentialist.push(`<div style="font-size:13px;padding:1px;margin-left:-6px;color:#DDD;vertical-align:bottom"><img loading='lazy' src="${IMAGE_REPO}/ui/potential/${i+2}.png" style="margin-top:-4px;width:20px;background:#222;border-radius:25%;padding:2px"> ${potentials[i]}</div>`)
            }
            ...
            $.each(opdataFull.skills,function(i,v){
                ...
                <li class='nav-item'>
                    ...
                    <p><img loading='lazy' class='ak-shadow skill-image notclickthrough' id='skill${i}image' src='${IMAGE_REPO}/skills/skill_icon_${skillIcon}.png' style='width: 20px;margin:-4px 6px 0px -5px'>Skill ${i+1}</p>
                    </button>
                </li>
                `)
                        ...
                        <span class='custom-span skillname notclickthrough'><div>${skillname}</div></span>
                            <div class='topright'>
                                <div style='margin-top:-10px;padding:10px'>
                                    <img loading='lazy' class='ak-shadow skill-image notclickthrough' id='skill${i}image' src='${IMAGE_REPO}/skills/skill_icon_${skillIcon}.png' style='width: 100%;'>
                                </div>
                            </div>
                        ...
                        <button class='btn horiz-small nav-link ${(num!=2 ? '' : 'active')} equiplink' data-toggle='pill' href='#equip${num}'>
                            <div style = "display:inline-block;text-align:center;">
                                <div style = "display:inline-block; height:40px">
                                    <img loading='lazy' class='equip-image' src='${IMAGE_REPO}/equip/shining/${currequip.equipShiningColor}_shining.png' style='width: 50px; margin: 0px -5px 0px -5px'></img>
                                    <div style = "display:inline-block; position:absolute;margin: -1px 0px 0px -35px">
                                        <img loading='lazy' class='equip-image' src='${IMAGE_REPO}/equip/type/${currequip.typeIcon.toLowerCase()}.png' style='width: 30px; height: 30px; position: absolute; aspect-ratio: 1; object-fit: contain;'></img>
                                    </div>
                        ...
                        $.each(currequip.itemCost,function(key,v){
                            ...
                            imagereq.push(`
                                <div style="color:#fff;font-size:13px;background:#444;display:inline-block;padding:2px;border-radius:2px">
                                    <img loading='lazy' src="${IMAGE_REPO}/ui/elite/${currequip.unlockEvolvePhase.slice(-1)}.png" style="width:20px;margin:-12px 0px -6px 0px" title="Elite ${currequip.unlockEvolvePhase.slice(-1)}">
                                </div>
                            `)
                            ...
                            <span class='custom-span equipname'><div>${currequipEN?currequipEN.uniEquipName:currequip.uniEquipName}</div></span>
                                <div class='equipimage'>
                                    <button type="button" class="btn ak-button" style="width:90px;height:90px" data-toggle="modal" data-target="#opmodulestory" onclick="GetModuleStory('${module}')">
                                        ...
                                        ${currequip.uniEquipId.search("001")!=-1?`<img loading='lazy' class='equip-image' id='equip${i}image_add' src='${IMAGE_REPO}/ui/subclass/sub_${opdataFull.subProfessionId}_icon.png'style='z-index: 1;  width: 21%;  top: 27.7%;  position: absolute;  left: 37.2%;  max-height: 21%;object-fit: contain;  filter: invert(5%) sepia(6%) saturate(913%) hue-rotate(12deg) brightness(83%) contrast(86%);'>`:""}
                                        <img loading='lazy' class='equip-image' id='equip${num}image' src='${IMAGE_REPO}/equip/icon/${currequip.uniEquipIcon.search("001")==-1?currequip.uniEquipIcon:"original"}.png' style='width: 90px;height:90px;object-fit:contain' onerror="this.src='extra/not_found.png';">
                                    </button>
                                </div>
                        </div>
        ...
        switch (mission.template) {
            ...
            case "EquipmentSkillCastStage":
                ...
                skillId.forEach(eachskillId =>{
                    ...
                    skillimg.push(`<@ba.kw><img loading='lazy' src="${IMAGE_REPO}/skills/skill_icon_${eachskillId}.png" style="max-width:20px;margin:2px">${skillname} ${skillnum}</>`)
                })
            ...
            case "EquipmentSkillCast":
                ...
                tl=`
                    ...
                    </br>Cast <@ba.kw><img loading='lazy' src="${IMAGE_REPO}/skills/skill_icon_${skillId}.png" style="max-width:20px;margin:2px"> ${skillname} ${skillnum}</> <@ba.kw>${mission.paramList[2]}</> times
                    `
                break;
            ...
            case "EquipmentEventStageKill":
                ...
                deathdetails.forEach(deathdetail =>{
                    ...
                    enemiesdetails.push(`<@ba.kw>${enemyName}</> <img loading='lazy' src="${IMAGE_REPO}/enemy/${enemyid}.png" style="max-width:50px"></img>`)
                })
                tl=`
                    Clear <@ba.kw>${stage}</> with <@ba.kw>${mission.paramList[0]}</> Star
            ...
            case "EquipmentEventTotal":
                ...
                switch (splitreq[splitreq.length-1]) {
                    ...
                    case "2":   // Ambriel
                        ...
                        enemytype = "Enemies"
                        wordtype = `with <@ba.kw> <img loading='lazy' src="${IMAGE_REPO}/skills/skill_icon_${skillid}.png" style="max-width:20px;margin:2px"> ${skillname} (Skill 2)</> </br>Using`
                        break;
                    case "ability":
                        enemytype = "enemies"
                ...
                if(splitreq.length>3){
                    if(db.skills[splitreq[2]]){
                        ...
                        skillnum = `(Skill ${skillnum+1})`
                        extra = `
                        's <@ba.kw><img loading='lazy' src="${IMAGE_REPO}/skills/skill_icon_${skillId}.png" style="max-width:20px;margin:2px"> ${skillname} ${skillnum}</>
                        `
                    }
                }
            ...
            case "EquipmentEventStageMore" :
                ...
                if(enemyid2){
                    ...
                    <img loading='lazy' src="${IMAGE_REPO}/enemy/${enemyid2}.png" style="max-width:50px">`
                }
                ...
                if(enemyName&&skill){
                    ...
                    <img loading='lazy' src="${IMAGE_REPO}/enemy/${enemyid}.png" style="max-width:50px">
                    </br>with <@ba.kw> <img loading='lazy' src="${IMAGE_REPO}/skills/skill_icon_${skill}.png" style="max-width:20px;margin:2px"> ${skillname} (Skill ${skillid})</>
                    ...
                    if(enemyName == "any"){
                        ...
                        tl=`
                            ...
                            <img loading='lazy' src="${IMAGE_REPO}/enemy/${enemyid}.png" style="max-width:50px"> ${objective2}
                            </br>Using Non-Borrowed <@ba.kw>${chara.appellation}</>
                        `
                    }
                }else if (skill){
                    tl=`
                    Clear <@ba.kw>${stage}</> with <@ba.kw>${mission.paramList[0]}</> Star
                    </br>${objective} <@ba.kw>${mission.paramList[4]}</> enemies using <@ba.kw> <img loading='lazy' src="${IMAGE_REPO}/skills/skill_icon_${skill}.png" style="max-width:20px;margin:2px"> ${skillname} (Skill ${skillid})</>
                    </br>Using Non-Borrowed <@ba.kw>${chara.appellation}</>
                    `
                }else if(chara2){
                    tl=`
            ...
            case "EquipmentSkillCastBattle" :
                ...
                skills.forEach(element => {
                    ...
                    skilltext += `<@ba.kw> <img loading='lazy' src="${IMAGE_REPO}/skills/skill_icon_${element.id}.png" style="max-width:20px;margin:2px"> ${element.name} (Skill ${element.num})</> </br>`
                });
                tl =`
                        Complete <@ba.kw>${mission.paramList[0]}</> stages </br>
                        ${skilltext}
            ...
            case "EquipmentBattleCharKilled" :
                ...
                splitChara.forEach(element => {
                    ...
                    splitName+=  `</br><@ba.kw> <img loading='lazy' src="${IMAGE_REPO}/avatars/${element}.png" style="max-width:50px"> ${currchar.appellation} </>`
                });
                tl =`
                        Complete <@ba.kw>${mission.paramList[0]}</> stages </br>
            ...
            case "EquipmentDeployCharAndKillCnt":
                ...
                tl =`
                        ...
                        <@ba.kw> <img loading='lazy' src="${IMAGE_REPO}/avatars/${mission.paramList[2]}.png" style="max-width:50px"> ${currchar.appellation} </>, and at least deployed <@ba.kw>${mission.paramList[1]}</> times
                    `
                break;
            case "EquipmentBattleCharDamage":
                ...
                tl =`
                        ...
                        <@ba.kw> <img loading='lazy' src="${IMAGE_REPO}/avatars/${mission.paramList[1]}.png" style="max-width:50px"> ${currchar.appellation} </>
                    `
                break;
    ...
    function UpdateTokenContent(wholetokenupdate = true){
        ...
        if (wholetokenupdate) {
            $("#token-contents").html(
                ...
                <div class="token-details" style='background:#333;margin:2px 0px;padding:2px 10px;height:64px'>
                    <div style ="display:inline-block;margin:0px 2px;background:#222222aa">
                        <img loading='lazy' class='token-image notclickthrough' id='Tokenimage' src='${IMAGE_REPO}/avatars/${globaltoken}.png' style='width:60px;margin:-0px 0px 0px 0px' onerror="this.src='extra/not_found.png';">
                    </div>
                    <div style ="position:absolute;left:80px;top:7px">
                        <div class='stats'>
                            <div class='stats-l' style="min-width:unset;width:40px;background:#3D3D3D"><img loading='lazy' style='max-height:30px;margin:-11px -10px -9px -10px' src='${IMAGE_REPO}/ui/elite/${currelite}-s.png'></div><div class='stats-r' style="min-width:unset">Lv <span id="summon-level">${currlevel}</span></div>
                        </div>
                        <div>
            ...
            $("#token-contents .token-details .stats").html(`<div class='stats-l' style="min-width:unset;width:40px;background:#3D3D3D"><img loading='lazy' style='max-height:30px;margin:-11px -10px -9px -10px' src='${IMAGE_REPO}/ui/elite/${currelite}-s.png'></div><div class='stats-r' style="min-width:unset">Lv <span id="summon-level">${currlevel}</span></div>`)
        }
    ...
    function UpdataTokenTalent(token = globaltoken){
        ...
        talentObject.req2.forEach(reqs => {
            ...
            if(!elitelevel.includes(elreq)){
                if(currphase >=0)
                    imagereq.push(`<img loading='lazy' src="${IMAGE_REPO}/ui/elite/${currphase}.png" style="width:18px;margin-top:-5px">`)
                if(currlevel >1)
        ...
        potential.forEach(currpotent => {
            var imagereq = []
            if(currpotent+1 >0)
                imagereq.push(`<img loading='lazy' src="${IMAGE_REPO}/ui/potential/${currpotent+1}.png" style="width:18px"> ${currpotent+1}`)
            talentTab2.push(`
    ...
    function GetModuleStory(module){
        ...
        $("#opmodulestorycontent").html(`
            ...
            <div class="equip-image-container">
                ${currequip.uniEquipId.search("001")!=-1?`<img loading='lazy' class='equip-image' id='equip${i}image_add' src='${IMAGE_REPO}/ui/subclass/sub_${opdataFull.subProfessionId}_icon.png'style='z-index: 1;  width: 21%;  top: 27.7%;  position: absolute;  left: 37.2%;  max-height: 21%;object-fit: contain;filter: invert(5%) sepia(6%) saturate(913%) hue-rotate(12deg) brightness(83%) contrast(86%);'>`:""}
                <img loading='lazy' class='equip-image' id='equip${i}image' src='${IMAGE_REPO}/equip/icon/${currequip.uniEquipIcon.search("001")==-1?currequip.uniEquipIcon:"original"}.png' style='width:100%;max-width:500px' onerror="this.src='extra/not_found.png';">
            </div>
    ...
    function getEliteHTML(i, opdataFull){
        ...
        if(i > 0){
            ...
            var mats = $(`<div class='small-container ak-shadow'>
                            ...
                            <img loading='lazy' class='topright' src='${IMAGE_REPO}/ui/elite/${i}.png' width='100'>
            ...
            eachcat.list.forEach(id => {
                ...
                <div class="" style="background:#444;margin:4px;padding:0px;border-radius:2px;text-align:left">
                    <div style="background:#999;display:inline-block;padding:2px;width:100%;border-radius:2px 2px 0px 0px;position:relative;height:33px">
                        <img loading='lazy' id="op-riicdetail-img" src="${IMAGE_REPO}/ui/infrastructure/skill/${currbuff.skillIcon}.png" style="border-radius:50%;background:#333;padding:3px;position:absolute;left:2px;top:2px;width:30px" title=""></img>
    ...
    function ModuleTalentParse(CN,TL,EN,modulenum,phase,currpotent,range_id=""){
        ...
        if(CN.requiredPotentialRank+1>0)
            imagereq.push(`<img loading='lazy' src="${IMAGE_REPO}/ui/potential/${CN.requiredPotentialRank+1}.png" style="width:20px" title="Potential ${CN.requiredPotentialRank+1}">`)
    ...
    function TalentParse2(eachtalent, talentnum, prefix = ""){
        ...
        if(PhaseConvert(eachtalent.talent.unlockCondition.phase) >=0)
            imagereq.push(`<img loading='lazy' src="${IMAGE_REPO}/ui/elite/${PhaseConvert(eachtalent.talent.unlockCondition.phase)}.png" style="width:20px;margin-top:-5px" title="Elite ${eachtalent.talent.unlockCondition.phase}">`)
        ...
        if(eachtalent.talent.requiredPotentialRank >0)
            imagereq.push(`<img loading='lazy' src="${IMAGE_REPO}/ui/potential/${eachtalent.talent.requiredPotentialRank+1}.png" style="width:20px" title="Potential ${eachtalent.talent.requiredPotentialRank+1}">`)
    ...
    function CreateMaterial(id, count){
        ...
        (`<div class="akmat-container" style="position:relative">
            ...
            <div class="item-image">
                <img loading='lazy' id="item-image" src="${IMAGE_REPO}/items/${itemdata.iconId}.png">
            </div>
            <img loading='lazy' class="item-rarity" src="${IMAGE_REPO}/material/bg/item-${RarityConvert(itemdata.rarity)+1}.png">
    ...
    function GetTrait(desc,trait,traitname = "Traits"){
        ...
        if(trait&&(trait.candidates.length>0)){
            ...
            trait.candidates.forEach(element => {
                ...
                imagereq.push(`<img loading='lazy' src="${IMAGE_REPO}/ui/elite/${element.unlockCondition.phase}.png" style="width:20px;margin:-12px 0px -6px 0px" title="Elite ${element.unlockCondition.phase}">`)
    ...
    function LoadAnimationCG(opid, dynid, isSkin = false){
        var dynfolder = `${SPINE_REPO}/spineassets/dynchars/${opid}/`
    ...
    function LoadAnimation2(widget){
        ...
        if (chibiName != null && defaultAnimationName != null) {
            var xhr = new XMLHttpRequest();
            folder2 = `${SPINE_REPO}/spineassets/dynchars/char_1012_skadi2/dyn_illust_char_1012_skadi2_2/dyn_illust_char_1012_skadi2`
