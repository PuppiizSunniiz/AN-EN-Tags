...
def load_json(data) -> dict :
    match data:
        ...
        case "Activityjson" :
            return json_load("test/Akenemy-NEW/activity.json")
...
def enemy_ability_list(json_Database : dict,json_DatabaseEN : dict,json_Handbook : dict, json_HandbookEN : dict) -> None :
    enemy_database_CN = {}
    enemy_database_EN = {}
    ...
    for enemy_key in json_Handbook["enemyData"]:
        ## Handbook Test
        if not json_Handbook["enemyData"][enemy_key]["hideInHandbook"] :
            continue
        else :
            database = enemy_database_EN if enemy_key in enemy_database_EN else enemy_database_CN
            handbook = json_HandbookEN["enemyData"] if enemy_key in json_HandbookEN["enemyData"] else json_Handbook["enemyData"]
        ...
        if len(database[enemy_key]) > 1:
            for key in ["applyWay","motion","enemyTags","lifePointReduce","rangeRadius"]:
                if database[enemy_key][1]["enemyData"][key]["m_defined"] and database[enemy_key][0]["enemyData"][key]["m_value"] != database[enemy_key][1]["enemyData"][key]["m_value"]:
                    print(enemy_key,"has {key} Level Conflict")
                    errortest += 1
        ...
        if len(database[enemy_key]) > 1 and abilitiescheck["all"]:
            if database[enemy_key][0]["enemyData"]["spData"] != database[enemy_key][1]["enemyData"]["spData"] and database[enemy_key][0]["enemyData"]["spData"] and database[enemy_key][1]["enemyData"]["spData"]:
                print(enemy_key,"has spData Level Conflict")
                errortest += 1
                if errortest == 1 : print("\n#########################################################\n###################     Error Test    ###################\n#########################################################")
    ...
    print("Enemy Tags (Handbook \"enemyTags\") = ",enemy_Tag)
    print("Enemy Motion = ",enemy_motion)
    print("Enemy Attack Type = ",enemy_applyWay)
    print("\n")
    
    
    with open("test/Akenemy-NEW/enemy_database_stat.json",'w') as JSON :
        json.dump(enemy_database,JSON,indent = 4, ensure_ascii = False)

    with open("test/Akenemy-NEW/enemy_database_abilities.json",'w') as JSON :
...
def IS_boss(stage_id : str, IS : str) -> str:
    ...
    return IS_boss_dict[IS][stage_id]
...
################################################################################################################################################################################################################################################
def json_dumps():
    with open("test/Akenemy-NEW/activity.json",'w') as JSON :
        json.dump(activity_collection, JSON, indent = 4, ensure_ascii = False)

    with open("test/Akenemy-NEW/stage.json",'w') as JSON :
        json.dump(stage_collection,  JSON, indent = 4, ensure_ascii = False)
    
    with open("test/Akenemy-NEW/akenemy.json",'w') as JSON :
...
def test_dumps(dump_collection):
    with open("test/Akenemy-NEW/test-dump.json",'w') as JSON :
