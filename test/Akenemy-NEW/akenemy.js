...
var d5 = $.getJSON("akenemy.json",function(data){
    db["akenemy"] = data;
});
...
function event_listing(gamemode){    // new
    ...
    function act_lister(act,zone){
        return `<li class="act_list">
                    <a class="dropdown-item unselectable" onclick="selectGameevent($(this))" act-id="${act}" zone-id="" stage-id="">
                        ${db.akenemy.events[act]} <i class="fa fa-caret-right"></i>
                    </a>
                    <ul class="dropdown-menu dropdown-submenu" style="margin-top: 5px;border:0">
                        ${zone}
                    </ul>
                </li>`
    }
    function zone_lister(act,zone,stage){
        return `<li class="zone_list">
                    <a class="dropdown-item unselectable" onclick="selectGameevent($(this))" act-id="${act}" zone-id="${zone}" stage-id="">
                        ${activity_dict[act].zone[zone].name} <i class="fa fa-caret-right"></i>
                    </a>
                    <ul class="dropdown-menu dropdown-submenu" style="margin-top: 5px;border:0">
                        ${stage}
                    </ul>
    ...
    function stage_lister(act,zone,stage){
        return `<li class="stage_list">
                    <a class="dropdown-item unselectable" onclick="selectGameevent($(this))" act-id="${act}" zone-id="${zone}" stage-id="${stage}">
                        ${activity_dict[act].zone[zone].stage[stage]}
                    </a>
                </li>`
    }

    let NoAct = false
    Object.keys(activity_dict).forEach(act =>{
        let zone_list = ""
        let NoZone = false
        Object.keys(activity_dict[act]["zone"]).forEach(zone =>{
            ...
            else zone_list += zone_lister(act, zone, stage_list)
        })
        if (NoAct) gameeventhtml += zone_list
        else gameeventhtml += act_lister(act, zone_list)
        NoAct = false
    })
...
function selectGameevent(el){
    $("#GameeventDropdown").html(el.html().replace(` <i class="fa fa-caret-right"></i>`,""))
    $("#GameeventDropdown").attr("act-id",el.attr("act-id"))
    $("#GameeventDropdown").attr("zone-id",el.attr("zone-id"))
    $("#GameeventDropdown").attr("stage-id",el.attr("stage-id"))
...
function populateEnemy(allenemies=false){
    ...
    let enemy_gamezone  = $("#GameeventDropdown.btn-primary").attr("zone-id")
    let enemy_gamestage = $("#GameeventDropdown.btn-primary").attr("stage-id")
